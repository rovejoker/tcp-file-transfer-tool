...
class XProtocol:
    ...
    def build_frame(frame_type: int, data: bytes) -> bytes:
        ...
        header = struct.pack(HEADER_FORMAT, VERSION, frame_type, data_length)
        return header + data
    ...
    def parse_header(header_data: bytes) -> tuple:
        ...
        Returns:
            tuple: (版本, 帧类型, 数据长度)
        """
        version, frame_type, data_length = struct.unpack(HEADER_FORMAT, header_data)
        return version, frame_type, data_length
